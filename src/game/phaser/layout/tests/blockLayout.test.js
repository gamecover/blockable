...
const block = (shape) => createBlock(shape, `layout-${shape}`)

describe('block layout', () => {
  it('lays out horizontal and vertical I blocks on three board cells', () => {
    expect(layoutBlockForBoard(block('I'), 0, board).cells.map(({ x, y, size }) => [x, y, size])).toEqual([[0, -54, 49], [0, 0, 49], [0, 54, 49]])
    expect(layoutBlockForBoard(block('I'), 1, board).cells.map(({ x, y, size }) => [x, y, size])).toEqual([[54, 0, 49], [0, 0, 49], [-54, 0, 49]])
  })
  ...
  it('restores the hand metrics', () => {
    ...
    expect(layout.cells.some(({ x }) => x === 17)).toBe(true)
  })

  it('keeps the board anchor offset separate from the centered visual coordinates', () => {
    expect(getBlockAnchorOffset(layoutBlockForBoard(block('I'), 0, board))).toEqual({ x: 0, y: 54 })
    expect(getBlockAnchorOffset(layoutBlockForBoard(block('I'), 1, board))).toEqual({ x: 54, y: 0 })
    expect(getBlockAnchorOffset(layoutBlockForBoard(block('O'), 0, board))).toEqual({ x: 27, y: 27 })
  })

  it.each([
    ['I', 0, { x: -15.5, y: -49.5, width: 31, height: 99 }],
    ['I', 1, { x: -49.5, y: -15.5, width: 99, height: 31 }],
  ...
  it('hit-tests the visible cells from the same centered coordinates used for rendering', () => {
    const verticalI = layoutBlockForHand(block('I'), 0, hand)
    const lBlock = layoutBlockForHand(block('L'), 0, hand)

    expect(isPointInsideBlock(verticalI, 0, -34)).toBe(true)
    expect(isPointInsideBlock(verticalI, 0, 0)).toBe(true)
    expect(isPointInsideBlock(verticalI, 0, 34)).toBe(true)
    expect(isPointInsideBlock(verticalI, 0, 66)).toBe(false)
