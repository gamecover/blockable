  ...
  return (
    <ScreenFrame title="전리품을 선택하세요" subtitle="BATTLE WON">
      ...
      <div className="reward-grid">
        {rewards.map((block) => <motion.button whileHover={{ y: -8 }} className="block-card" key={block.id} onClick={() => onChoose(block)}><BlockPreview block={block} /><strong>{block.name}</strong><small>{block.description}</small><em>도구 주머니에 추가</em></motion.button>)}
      </div>
